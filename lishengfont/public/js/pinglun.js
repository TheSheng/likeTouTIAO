(function($){
	...
	function crateCommentInfo(obj){
		...
		if(typeof(obj.osname) != "undefined" && obj.osname != ""&&null!=obj.id){
			el =el+"<span><i class='glyphicon glyphicon-pushpin'></i><i class='lou'>"+obj.id+"</i>楼来自:"+obj.osname+"</span>";
		}
	...
	function replyClick(el){
		...
		el.parent().parent().append("<div class='replybox'><textarea   cols='80' rows='50' placeholder='来说几句吧......' class='mytextarea' ></textarea><span class='send' ng-click='huifu()'>发送</span></div>")
		...
		.find(".send").click(function(){
			...
			if(content != ""){
				...
				obj.id=el.parent().parent().find(".lou").text()
                if(obj.id==""||obj.id==null){
                	console.log(obj.id=el.parent().parent().parent().find(".lou").text())
					console.log(obj.id=el.parent().parent().parent().parent().find(".lou").text())
				}
				obj.userId=$(".userId").text()
				obj.articleId=$(".newsId").text()

				console.log(obj)
				$.ajax({
					type:"POST",
					url:"http://129.211.63.167:8080/services/news/huifu",
